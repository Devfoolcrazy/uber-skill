...
### 13. Suggérer des skills pour un projet : implémenté, en attente du test utilisateur

- Objectif (demande du 23 septembre 2026) : la bibliothèque grossit ; un bouton demande à Claude quels skills et agents conviennent à un projet, à partir d'un scan du dossier ou de quelques réponses si le dossier est vierge, et présélectionne ce qu'il propose.
- Décisions validées : même mécanisme que « Raffiner… » (`claude -p` sans outil, Claude seulement) ; la fenêtre dit ce qui part avant de demander ; envoyé : `INDEX.md`, arborescence sur trois niveaux sans dépendances ni sorties de build, texte tronqué des fichiers de racine connus, liste des éléments déjà installés, trois réponses facultatives (type, stack, objectif) posées par l'app puisque `claude -p` est un aller simple ; aucun fichier source ; réponse JSON vérifiée contre la bibliothèque (identifiants inconnus ignorés et listés) ; présélection cochable avec raison et confiance, jamais d'installation automatique : « Installer la sélection » passe par la vérification d'installation habituelle ; agents proposés aussi.
- Réalisé : module `suggest` du `core` (`gather` pour ce qui part, `suggest` pour la demande, vérification de la réponse), appel `claude -p` partagé avec le raffinement, code d'erreur `suggest.invalid-answer`, commandes Tauri `suggest_context` et `suggest_items`, modale « Suggérer des skills… » dans la vue Projets avec choix de la cible, commande CLI `suggest` (`--kind`, `--stack`, `--goal`, `--dry-run`, `--json`), documentation (référence, guide, README).
- Vérifications effectuées : suite Rust complète (62 tests, dont 3 sur la suggestion : scan sans dépendances ni code source, vérification de la réponse, faux `claude` qui enregistre ce qu'il reçoit), `pnpm check` sans erreur ni avertissement, 65 tests d'interface (dont 4 sur la modale), et une demande réelle en ligne de commande sur ce dépôt : 20 secondes, 9 propositions pertinentes avec des raisons concrètes (tlc-plan, codebase-design, diagnosing-bugs, reviewer, coder…). L'interface n'a pas été essayée dans l'application réelle.
- À vérifier par l'utilisateur : ouvrir Projets, choisir un projet, « Suggérer des skills… » ; lire ce qui sera envoyé, remplir ou non les trois champs, lancer ; vérifier la liste, décocher un élément, « Installer la sélection » doit ouvrir la vérification d'installation habituelle ; essayer sur un dossier vide (le message doit inviter à décrire le projet) ; essayer « Abandonner » pendant l'attente.
- Limites : un seul appel, pas de dialogue avec Claude ; pas de mémoire des suggestions refusées ; le scan ne lit que la racine du projet (un monorepo n'expose que ses manifestes de racine) ; pas de suggestion pour l'installation globale.
