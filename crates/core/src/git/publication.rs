...
pub(super) fn optional(cmd: &mut Command) -> Result<Option<String>> {
    ...
    if output.status.success() {
        Ok(Some(String::from_utf8_lossy(&output.stdout).trim_end_matches(['\r', '\n']).to_owned()))
    } else {
        Ok(None)
    }
}
...
pub fn preview(path: &Path) -> Result<Preview> {
    ...
    let remote_branch = remote_ref.as_deref().and_then(|r| r.strip_prefix("refs/heads/")).map(str::to_owned);
    let mut blocked = if branch.is_none() {
        ...
        Some("Cette branche n’a pas de branche distante de suivi. Configurez-la dans votre outil Git avant de publier.".into())
    } else { None };
    ...
    if let Some(h) = &head {
        let range = upstream.as_ref().map(|u| format!("{u}..{h}")).unwrap_or_else(|| h.clone());
        pending_count = run(git(&root).args(["rev-list", "--count", &range]))?.parse().map_err(|_| Error::Git("Nombre de commits Git invalide.".into()))?;
        pending_commits = run(git(&root).args(["log", "--format=%h %s", "-20", &range, "--"]))?.lines().map(str::to_owned).collect();
    }
    ...
    match &head {
        Some(h) => { run(temp_git().args(["read-tree", h]))?; }
        None => { run(temp_git().args(["read-tree", "--empty"]))?; }
    }
    ...
    for pair in parts.chunks_exact(2) {
        let diff = run(temp_git().args(["diff", "--cached", "--no-ext-diff", "--no-textconv", "--no-color", "--no-renames", "--", pair[1]]))?;
        if diff.contains("160000") && run(temp_git().args(["diff", "--cached", "--raw", "--", pair[1]]))?.contains("160000") {
            blocked = Some("La publication de changements de sous-modules doit être effectuée dans votre outil Git.".into());
        }
        files.push(ChangedFile { path: pair[1].to_owned(), status: pair[0].to_owned(), diff });
    }
    let mut preview = Preview {
        root: root.to_string_lossy().into_owned(), branch, head, remote, remote_branch,
        pending_count, pending_commits, files, snapshot: String::new(), blocked,
    };
...
pub fn publish(path: &Path, snapshot: &str, paths: &[String], message: &str) -> Result<PublicationResult> {
    let _guard = super::OPERATION_LOCK.lock().map_err(|_| Error::Git("Publication Git indisponible.".into()))?;
    let current = preview(path)?;
    if current.snapshot != snapshot {
        return Err(Error::Git("La bibliothèque a changé depuis l’aperçu. Actualisez les différences avant de publier.".into()));
    }
    ...
    if selected.len() != paths.len() || selected.iter().any(|p| !current.files.iter().any(|f| &f.path == *p)) {
        return Err(Error::Git("La sélection contient un fichier absent de l’aperçu.".into()));
    }
    ...
    if paths.is_empty() && current.pending_count == 0 {
        return Err(Error::Git("Sélectionnez des fichiers à publier. Aucun commit local n’est en attente.".into()));
    }
    ...
    let head = commit.as_ref().or(current.head.as_ref()).ok_or_else(|| Error::Git("Aucun commit à envoyer.".into()))?;
    ...
    let push = run(git(root)
        .args(["-c", &format!("remote.{remote}.mirror=false"), "push", "--no-follow-tags", "--recurse-submodules=no", "--", remote, &format!("{head}:refs/heads/{remote_branch}")]));
    Ok(PublicationResult { commit, pushed: push.is_ok(), push_error: push.err().map(|e| e.to_string()) })
}
...
pub(super) mod tests {
    ...
    pub(in crate::git) fn fixture() -> (TempDir, PathBuf, PathBuf) {
        ...
        for file in ["a.md", "b.md", "delete.md"] { fs::write(local.join(file), "initial\n").unwrap(); }
        run_git(&local, &["add", "."]);
    ...
    fn selected_files_only_preserving_other_staging_and_preview_index() {
        ...
        assert!(p.files.iter().any(|f| f.path == special && f.diff.contains("+new file")));
        let result = publish(&local, &p.snapshot, &["a.md".into(), "delete.md".into(), special.into()], "Selected changes").unwrap();
    ...
    fn rejects_stale_preview_and_unreviewed_paths_before_committing() {
        ...
        assert!(publish(&local, &p.snapshot, &["a.md".into()], "stale").unwrap_err().to_string().contains("a changé"));
        let p = preview(&local).unwrap();
        for paths in [vec!["../escape".into()], vec!["*".into()], vec!["a.md".into(), "a.md".into()]] {
    ...
    fn failed_push_keeps_commit_and_retry_does_not_create_another() {
        ...
        run_git(&local, &["remote", "set-url", "--push", "origin", temp.path().join("missing.git").to_str().unwrap()]);
        ...
        run_git(&local, &["remote", "set-url", "--push", "origin", remote.to_str().unwrap()]);
        let p = preview(&local).unwrap();
    ...
    fn distant_advance_rejects_push_without_force() {
        ...
        run_git(temp.path(), &["clone", remote.to_str().unwrap(), other.to_str().unwrap()]);
        fs::write(other.join("remote-only.md"), "remote\n").unwrap();
        run_git(&other, &["add", "."]);
        run_git(&other, &["-c", "user.name=Test", "-c", "user.email=test@example.com", "-c", "commit.gpgSign=false", "commit", "-m", "remote advance"]);
        run_git(&other, &["push"]);
    ...
    fn rejected_commit_hook_preserves_files_and_does_not_push() {
        ...
        let p = preview(&local).unwrap();
        let error = publish(&local, &p.snapshot, &["a.md".into()], "rejected").unwrap_err().to_string();
    ...
    fn binary_additions_and_renames_are_reviewable_and_publishable() {
        ...
        let p = preview(&local).unwrap();
        assert!(p.files.iter().any(|f| f.path == "asset.bin" && f.diff.contains("Binary files")));
        assert!(p.files.iter().any(|f| f.path == "a.md" && f.status == "D"));
        assert!(p.files.iter().any(|f| f.path == "renamed.md" && f.status == "A"));
        let result = publish(&local, &p.snapshot, &["a.md".into(), "renamed.md".into(), "asset.bin".into()], "rename and binary").unwrap();
        assert!(result.pushed);
        assert_eq!(run_git(&remote, &["show", "main:renamed.md"]), "initial");
        assert_eq!(git(&remote).args(["show", "main:asset.bin"]).output().unwrap().stdout, binary);
    }
}
