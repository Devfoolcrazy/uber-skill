...
use super::publication::{git, optional, config};
...
pub fn inspect(path: &Path) -> Result<SyncStatus> {
    ...
    let mut status = SyncStatus {
        root: root.to_string_lossy().into_owned(), branch, head,
        remote: None, remote_ref: None, upstream_ref: None, remote_head: None,
        ahead: 0, behind: 0, changed_files: Vec::new(), verified: false,
        fetch_error: None, blocked: None, snapshot: String::new(),
    };
    if let Some(branch) = &status.branch {
        ...
        if !tracking.is_empty() { status.upstream_ref = Some(tracking); }
    } else {
    ...
    if status.remote.as_deref().is_none_or(|r| r.is_empty() || r == ".")
        || status.remote_ref.as_deref().is_none_or(|r| !r.starts_with("refs/heads/"))
        || status.upstream_ref.as_deref().is_none_or(|r| !r.starts_with("refs/remotes/"))
    {
        status.blocked.get_or_insert("Aucune branche distante de suivi utilisable. Configurez-la dans votre outil Git.".into());
    }
    ...
    let changed = run(git(&root).args(["status", "--porcelain=v1", "--no-renames", "--untracked-files=all", "-z"]))?;
    status.changed_files = changed.split('\0').filter_map(|s| s.get(3..)).map(str::to_owned).collect();
    ...
    if let (Some(head), Some(upstream)) = (&status.head, &status.remote_head) {
        ...
        let counts: Vec<usize> = counts.split_whitespace().map(str::parse).collect::<std::result::Result<_, _>>()
            .map_err(|_| Error::Git("État de synchronisation Git invalide.".into()))?;
        if counts.len() != 2 { return Err(Error::Git("État de synchronisation Git incomplet.".into())); }
        status.ahead = counts[0]; status.behind = counts[1];
        if status.ahead > 0 && status.behind > 0 {
            status.blocked = Some("Les historiques local et distant divergent. Résolvez cette divergence dans votre outil Git.".into());
        }
    }
...
fn check_unlocked(path: &Path) -> Result<SyncStatus> {
    ...
    if let (Some(remote), Some(reference), Some(tracking)) = (&state.remote, &state.remote_ref, &state.upstream_ref) {
        if remote != "." && reference.starts_with("refs/heads/") && tracking.starts_with("refs/remotes/") {
            ...
            match run(git(path).args(["fetch", "--no-tags", "--no-recurse-submodules", "--", remote, &format!("+{reference}:{tracking}")])) {
                Ok(_) => { state = inspect(path)?; state.verified = state.remote_head.is_some(); }
                Err(e) => { state.fetch_error = Some(e.to_string()); }
            }
        }
    }
    Ok(state)
...
pub fn update(path: &Path, expected_snapshot: &str) -> Result<SyncStatus> {
    ...
    if state.snapshot != expected_snapshot {
        return Err(Error::Git("L’état Git a changé. Vérifiez à nouveau avant de mettre à jour.".into()));
    }
    if !state.verified { return Err(Error::Git("La fraîcheur distante n’a pas pu être vérifiée. Aucune mise à jour appliquée.".into())); }
    if let Some(reason) = &state.blocked { return Err(Error::Git(reason.clone())); }
    if state.behind > 0 {
...
pub(super) mod tests {
    ...
    fn fetch_then_fast_forward_preserves_unrelated_staged_and_untracked_changes() {
        ...
        assert_eq!(fs::read_to_string(local.join("untracked.md")).unwrap(), "untracked draft\n");
        assert_eq!(run_git(&local, &["rev-parse", "HEAD"]), run_git(&remote, &["rev-parse", "main"]));
    }
    ...
    fn offline_deleted_branch_and_changed_remote_do_not_look_verified() {
        ...
        assert!(update(&local, &reviewed.snapshot).unwrap_err().to_string().contains("a changé"));
        assert_eq!(fs::read_to_string(local.join("a.md")).unwrap(), "initial\n");
        run_git(&local, &["remote", "set-url", "origin", temp.path().join("missing.git").to_str().unwrap()]);
