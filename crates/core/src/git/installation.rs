...
fn source_state(skill: &Skill, status: Option<&sync::SyncStatus>) -> Result<SourceState> {
    let Some(status) = status else { return Ok(SourceState::Unverified) };
    let root = Path::new(&status.root);
    let Ok(relative) = skill.path.strip_prefix(root) else { return Ok(SourceState::Unverified) };
    let dirty = run(git(root).args(["status", "--porcelain=v1", "--no-renames", "--untracked-files=all", "--ignored", "-z", "--"]).arg(relative))?;
    let has_copied_changes = dirty.split('\0').filter_map(|record| record.get(3..)).any(|path| {
        !Path::new(path).components().any(|part| crate::fsutil::is_ignored(&part.as_os_str().to_string_lossy()))
    });
    if has_copied_changes { return Ok(SourceState::LocalDraft); }
    if let (Some(head), Some(upstream)) = (&status.head, &status.remote_head) {
        let commits = run(git(root).args(["log", "-1", "--format=%H", &format!("{upstream}..{head}"), "--"]).arg(relative))?;
        if !commits.is_empty() { return Ok(SourceState::UnpublishedCommit); }
    }
    Ok(if status.verified { SourceState::Published } else { SourceState::Unverified })
}

pub fn prepare(cfg: &Config, kind: ItemKind, ids: &[String], target: &Target) -> Result<InstallationPlan> {
    if ids.is_empty() { return Err(Error::Git("Aucun élément sélectionné.".into())); }
    if !target.supports(kind) { return Err(Error::Unsupported("Cette cible ne gère pas ce type d’élément.".into())); }
    ...
    for id in ids {
        ...
        if let Some(warning) = install::host_mismatch(&skill, target) { warnings.push(warning); }
        let state = source_state(&skill, git.as_ref())?;
        items.push(PreparedItem { id: id.clone(), source: skill.path, hash: skill.hash, source_state: state });
    }
    Ok(InstallationPlan { root: root.to_string_lossy().into_owned(), kind, target: target.clone(), items, git, git_error, warnings })
}

pub fn install_prepared(cfg: &Config, plan: &InstallationPlan, project: &Path) -> Result<Vec<LockEntry>> {
    let _guard = OPERATION_LOCK.lock().map_err(|_| Error::Git("Git indisponible.".into()))?;
    if cfg.library_path()?.to_string_lossy() != plan.root {
        return Err(Error::Git("La bibliothèque a changé. Recommencez la vérification.".into()));
    }
    if let Some(expected) = &plan.git {
        if sync::inspect(Path::new(&plan.root))?.snapshot != expected.snapshot {
            return Err(Error::Git("L’état Git a changé depuis la vérification. Actualisez avant d’installer.".into()));
        }
    }
    ...
    for expected in &plan.items {
        ...
        if skill.hash != expected.hash || skill.path != expected.source {
            return Err(Error::Git(format!("{} a changé depuis la vérification. Actualisez avant d’installer.", skill.id)));
        }
        skills.push(skill);
    }
    skills.iter().zip(&plan.items).map(|(skill, item)| {
        install::install_with_state(skill, project, &plan.target, Some(item.source_state))
    }).collect()
}
...
mod tests {
    ...
    use super::super::sync::tests::{writer, push_file};
    ...
    fn seed(local: &Path) -> Config {
        ...
        Library::open_kind(local.join("agents"), ItemKind::Agent).unwrap().create("reviewer", "Review", None, &[], &[]).unwrap();
        run_git(local, &["add", "."]);
        run_git(local, &["commit", "-m", "seed items"]);
        run_git(local, &["push"]);
        Config { library_path: Some(local.to_path_buf()), ..Config::default() }
    }
    ...
    fn installs_explicit_local_draft_offline_and_persists_source_state() {
        ...
        let cfg = seed(&local);
        run_git(&local, &["remote", "set-url", "origin", temp.path().join("unavailable.git").to_str().unwrap()]);
        fs::write(local.join("skills/one/draft.txt"), "local draft\n").unwrap();
        let plan = prepare(&cfg, ItemKind::Skill, &["one".into(), "two".into()], &Target::ClaudeCode).unwrap();
        ...
        assert_eq!(fs::read_to_string(project.join(".claude/skills/one/draft.txt")).unwrap(), "local draft\n");
    ...
    fn updates_then_reprepares_and_installs_new_content_for_skills_and_agents() {
        ...
        push_file(&writer, "agents/reviewer.md", "---\nname: reviewer\ndescription: New reviewer\n---\nNew instructions\n");
        ...
        assert_eq!(fs::read_to_string(project.join(".claude/skills/one/instructions.txt")).unwrap(), "new remote instructions\n");
        let plan = prepare(&cfg, ItemKind::Agent, &["reviewer".into()], &Target::ClaudeCode).unwrap();
        install_prepared(&cfg, &plan, &project).unwrap();
        assert!(fs::read_to_string(project.join(".claude/agents/reviewer.md")).unwrap().contains("New instructions"));
    }
    ...
    fn rejects_changed_batch_before_copying_and_recognizes_unpublished_commits() {
        ...
        assert!(!project.exists());
        let old: LockEntry = serde_json::from_str(r#"{"id":"one","source":"/old/one","hash":"hash","installed_at":"date"}"#).unwrap();
